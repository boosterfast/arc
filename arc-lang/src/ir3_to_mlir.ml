...
and lower_stmt (SVal (v, t, e)) ctx : Ctx.t =
  ...
  match e with
  | EAccessRecord (info, v1, x1) ->
      let (a, ctx) = ctx |> lower_var v1 in
      ctx |> Ctx.add_ssa p (Mlir.OAccess (info, a, x1))
  | EBreak (info, v) ->
      let (a, ctx) = ctx |> lower_var_opt v in
      ctx |> Ctx.add_ssa p (Mlir.OBreak (info, a))
  | ECallExpr (info, v, vs) ->
      let (a, ctx) = ctx |> lower_var v in
      let (args, ctx) = vs |> mapm_filter lower_var_opt ctx in
      ctx |> Ctx.add_ssa p (Mlir.OCallExpr (info, a, args))
  | ECallItem (info, xs, ts, vs) ->
      let (x, ctx) = lower_path (xs, ts) ctx in
      let (args, ctx) = vs |> mapm_filter lower_var_opt ctx in
      ctx |> Ctx.add_ssa p (Mlir.OCallItem (info, x, args))
  | EAnnot _ ->
      todo ()
  | EContinue info ->
      ctx |> Ctx.add_ssa p (Mlir.OContinue info)
  | EEnwrap (info, x, v) ->
      let (a, ctx) = ctx |> lower_var_opt v in
      ctx |> Ctx.add_ssa p (Mlir.OEnwrap (info, x, a))
  | EUnwrap (info, x, v) ->
      let (a, ctx) = ctx |> lower_var v in
      ctx |> Ctx.add_ssa p (Mlir.OUnwrap (info, x, a))
  | ECheck (info, x, v) ->
      let (a, ctx) = ctx |> lower_var v in
      ctx |> Ctx.add_ssa p (Mlir.OCheck (info, x, a))
  | EItem (info, xs, ts) ->
      let (x, ctx) = lower_path (xs, ts) ctx in
      ctx |> Ctx.add_ssa p (Mlir.OConst (info, Mlir.CFun x))
  | EIf (info, v, b0, b1) ->
      let (a, ctx) = ctx |> lower_var v in
      let (b0, ctx) = ctx |> lower_block b0 result in
      let (b1, ctx) = ctx |> lower_block b1 result in
      ctx |> Ctx.add_ssa p (Mlir.OIf (info, a, b0, b1))
  | ELit (info, l) ->
      begin match l with
      | LInt (_, d) ->
          ctx |> Ctx.add_ssa p (Mlir.OConst (info, Mlir.CInt d))
      | LFloat (_, f) ->
          ctx |> Ctx.add_ssa p (Mlir.OConst (info, Mlir.CFloat f))
      | LBool (_, b) ->
          ctx |> Ctx.add_ssa p (Mlir.OConst (info, Mlir.CBool b))
      | LString (_, s) ->
          ctx |> Ctx.add_ssa p (Mlir.OConst (info, Mlir.CAdt (Printf.sprintf "\\\"%s\\\"" s)))
      | LUnit _ ->
          ctx
      | LChar (_, c) ->
          ctx |> Ctx.add_ssa p (Mlir.OConst (info, Mlir.CAdt (Printf.sprintf "'%c'" c)))
      end
  | ELoop (info, b) ->
      let (b, ctx) = ctx |> lower_block b yield in
      ctx |> Ctx.add_ssa p (Mlir.OLoop (info, b))
  | ERecord (info, vts) ->
      let (vts, ctx) = vts |> mapm_filter lower_field_expr_opt ctx in
      let (xs, ts) = vts |> map snd |> Utils.unzip in
      ctx |> Ctx.add_ssa p (Mlir.ORecord (info, xs, ts))
  | EReturn (info, v) ->
      let (a, ctx) = ctx |> lower_var_opt v in
      ctx |> Ctx.add_ssa p (Mlir.OReturn (info, a))
  | EUpdateRecord (info, v0, x, v1) ->
      let (a0, ctx) = ctx |> lower_var v0 in
      let (a1, ctx) = ctx |> lower_var v1 in
      ctx |> Ctx.add_ssa p (Mlir.OUpdate (info, a0, x, a1))
  | EBif _ -> todo ()
...
and return v ctx =
  let a = ctx |> Ctx.find_v_opt v in
  ctx |> Ctx.add_ssa None (Mlir.OReturn (gen, a))

and return_none _ ctx =
  ctx |> Ctx.add_ssa None (Mlir.OReturn (gen, None))

and result v ctx =
  let a = ctx |> Ctx.find_v_opt v in
  ctx |> Ctx.add_ssa None (Mlir.OResult (gen, a))

and yield _ ctx =
  ctx |> Ctx.add_ssa None (Mlir.OYield gen)
...
and lower_item ((xs, ts), i) ctx =
  ...
  | IDef (info, _, ps, t, b) ->
      ...
      ctx |> Ctx.add_item x (Mlir.IFunc (info, ps, t, b))
  | IAbstractDef (info, d, _, ts, t, effects) ->
      if is_defined_in_mlir d then
        ...
        let (t, ctx) = lower_type t ctx in
        ctx |> Ctx.add_item x (Mlir.IExternFunc (info, x_rust, ps, t, effects))
  | IVal _ ->
      todo ()
  | IAbstractType _ | IType _ -> ctx
...
and lower_item_extern_def_path x d ctx =
  ...
  | Some Some Ast.CString (_, x) -> (x, ctx)
...
and lower_type_item xs ts ctx =
  ...
  | Ir3.IAbstractType (_, d, _) ->
    begin match d |> List.assoc_opt "mlir" with
    | Some Some Ast.CString (_, x) ->
        (Mlir.TNative x, ctx)
    | None ->
