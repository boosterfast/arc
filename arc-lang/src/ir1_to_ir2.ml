(* This module implements a pattern compilation lowering and SSA. *)
open Utils
open Ir1
...
and lower_item (xs, i) ctx =
  match i with
  | IDef (info, a, gs, ps, t, _bs, b) ->
      let (ps, ctx) = ps |> mapm lower_param ctx in
      let (t, ctx) = ctx |> lower_type t in
      let (b, ctx) = ctx |> lower_block b in
      ctx |> Ctx.add_item xs (Ir2.IDef (info, a, gs, ps, t, b))
  | IAbstractDef (info, d, a, gs, ts, t, efs, _bs) ->
      let (t, ctx) = ctx |> lower_type t in
      let (ts, ctx) = ts |> mapm lower_type ctx in
      ctx |> Ctx.add_item xs (Ir2.IAbstractDef (info, d, a, gs, ts, t, efs))
  | IAbstractType (info, d, a, gs, _bs) ->
      ctx |> Ctx.add_item xs (Ir2.IAbstractType (info, d, a, gs))
  | IType (info, d, gs, t, _bs) ->
      let (t, ctx) = ctx |> lower_type t in
      ctx |> Ctx.add_item xs (Ir2.IType (info, d, gs, t))
  | IVal (info, d, t, b) ->
      let (t, ctx) = ctx |> lower_type t in
      let (b, ctx) = ctx |> lower_block b in
      ctx |> Ctx.add_item xs (Ir2.IVal (info, d, t, b))
  | IClass _ -> todo ()
...
and lower_expr e ctx =
  ...
  match e with
  | EAccessRecord (info, _, e1, x) ->
      let (v1, ctx) = ctx |> lower_expr e1 in
      ctx |> Ctx.add_expr t (Ir2.EAccessRecord (info, v1, x))
  | EUpdateRecord (info, _, e1, x, e2) ->
      let (v1, ctx) = ctx |> lower_expr e1 in
      let (v2, ctx) = ctx |> lower_expr e2 in
      ctx |> Ctx.add_expr t (Ir2.EUpdateRecord (info, v1, x, v2))
  | EAnnot (_, _, e1) ->
      ctx |> lower_expr e1
  | ELit (info, _, l) ->
      let l = lower_lit l in
      ctx |> Ctx.add_expr t (Ir2.ELit (info, l))
  | ELoop (info, _, b) ->
      let (b, ctx) = ctx |> lower_block b in
      ctx |> Ctx.add_expr t (Ir2.ELoop (info, b))
  | ERecord (info, _, (xes, e)) ->
      let (xes, ctx) = xes |> mapm lower_expr_field ctx in
      let (e, ctx) = ctx |> lower_expr_opt e in
      ctx |> Ctx.add_expr t (Ir2.ERecord (info, (xes, e)))
  | EReturn (info, _, e1) ->
      let (v1, ctx) = ctx |> lower_expr e1 in
      ctx |> Ctx.add_expr t (Ir2.EReturn (info, v1))
  | EBreak (info, _, e1) ->
      let (v1, ctx) = ctx |> lower_expr e1 in
      ctx |> Ctx.add_expr t (Ir2.EBreak (info, v1))
  | EContinue (info, _) ->
      ctx |> Ctx.add_expr t (Ir2.EContinue (info))
  | EEnwrap (info, _, xs, e1) ->
      let (v1, ctx) = ctx |> lower_expr e1 in
      ctx |> Ctx.add_expr t (Ir2.EEnwrap (info, xs, v1))
  | EItem (info, _, xs, ts) ->
      let (ts, ctx) = ts |> mapm lower_type ctx in
      ctx |> Ctx.add_expr t (Ir2.EItem (info, xs, ts))
  | ECallExpr (info, _, e1, es) ->
      let (v1, ctx) = ctx |> lower_expr e1 in
      let (vs, ctx) = es |> mapm lower_expr ctx in
      ctx |> Ctx.add_expr t (Ir2.ECallExpr (info, v1, vs))
  | ECallItem (info, _, xs, ts, es) ->
      let (ts, ctx) = ts |> mapm lower_type ctx in
      let (vs, ctx) = es |> mapm lower_expr ctx in
      ctx |> Ctx.add_expr t (Ir2.ECallItem (info, xs, ts, vs))
  | EVar (_, _, x) ->
...
and lower_lit l =
  match l with
  | LBool (info, l) -> Ir2.LBool (info, l)
  | LChar (info, l) -> Ir2.LChar (info, l)
  | LFloat (info, l) -> Ir2.LFloat (info, l)
  | LInt (info, l) -> Ir2.LInt (info, l)
  | LString (info, l) -> Ir2.LString (info, l)
  | LUnit info -> Ir2.LUnit info
...
and simplify_pat_field info v ((eqs, substs, b), (cs, ctx)) (x, p) =
  let (t, ctx) = ctx |> lower_type (typeof_pat p) in
  let (v, ctx) = ctx |> Ctx.add_expr t (Ir2.EAccessRecord (info, v, x)) in
...
and simplify_pat_eq ((eqs, substs, b), (cs, ctx)) (v, p) =
  ...
  | Ir1.PIgnore _ ->
      ((eqs, substs, b), (cs, ctx))
  | Ir1.PRecord (info, _, (xps, _p)) ->
      xps |> foldl (simplify_pat_field info v) ((eqs, substs, b), (cs, ctx))
  | Ir1.PAnnot (_, _, p) ->
      simplify_pat_eq ((eqs, substs, b), (cs, ctx)) (v, p)
  | Ir1.PVar (_, _, v0) ->
...
and lower_pat_clauses match_t clauses ctx =
  ...
  if eqs = [] then (* This pattern equation is now solved *)
    ...
    | Ir1.PUnwrap (info, _, variant_x, p) ->
        let (unwrap_t, ctx) = ctx |> lower_type (typeof_pat p) in
        branch_pat_unwrap_clause match_t info branch_v unwrap_t variant_x clauses ctx
    | Ir1.PConst (info, t, l) ->
        let (const_t, ctx) = ctx |> lower_type t in
        let const_l = l in
        branch_pat_const_clause match_t info branch_v const_t const_l clauses ctx
    (* Irrefutable top-level patterns are eliminated earlier through simplify_clauses *)
    | Ir1.PVar _ | Ir1.PIgnore _ | Ir1.PRecord _ | Ir1.POr _ | Ir1.PAnnot _ -> unreachable ()
...
and branch_pat_unwrap_clause match_t info branch_v unwrap_t variant_x clauses ctx =
  (* Create fresh variable for the inner pattern *)
  let (unwrap_v, ctx) = ctx |> Ctx.fresh_x in
  let unwrap_e = Ir2.EUnwrap (info, variant_x, branch_v) in
  ...
  | (then_clauses, else_clauses) ->
      let (v_check, ctx) = ctx |> Ctx.add_expr (Ir2.atom "bool") (Ir2.ECheck (info, variant_x, branch_v)) in
      ...
      let (v, ctx) = ctx |> Ctx.add_expr match_t (Ir2.EIf (info, v_check, then_b, else_b)) in
      (([], v), ctx)

and branch_pat_const_clause match_t info branch_v const_t const_l clauses ctx =
  let (then_clauses, else_clauses, ctx) = ctx |> split_pat_const_clauses clauses branch_v const_l in
  let (v, ctx) = ctx |> Ctx.add_expr (Ir2.atom "bool") (Ir2.ECallItem (info, ["std"; "eq"], [const_t], [branch_v])) in
  ...
  | (then_clauses, else_clauses) ->
      ...
      let (else_b, ctx) = lower_pat_clauses match_t else_clauses ctx in
      let (v, ctx) = ctx |> Ctx.add_expr match_t (Ir2.EIf (info, v, then_b, else_b)) in
      (([], v), ctx)
