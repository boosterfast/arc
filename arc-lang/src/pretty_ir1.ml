...
and pr_item (xs, i) ctx =
  ...
  | IAbstractDef (_, d, a, gs, ts, t, efs, bs) ->
      Pretty_ast.pr_decorator d ctx;
...
and pr_expr e ctx =
  let pr_expr e ctx =
    match e with
    | EAccessRecord (_, _, e, x) ->
        pr_expr e ctx;
        pr ".";
        pr_name x ctx;
    | EUpdateRecord (_, _, v0, x, v1) ->
        pr_expr v0 ctx;
        pr ".";
        pr_name x ctx;
        pr " = ";
