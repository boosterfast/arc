(* This module implements a monomorphisation lowering. *)
open Utils
open Ir2
...
module Ctx = struct
  ...
  and iscope = { isubst: (Ir2.generic * Ir3.ty) list; }
...
and lower_item (xs, i) ctx =
  ...
  | IAbstractDef (loc, d, a, [], ts, t, efs) ->
      let (t, ctx) = ctx |> lower_type t in
      let (ts, ctx) = ts |> mapm lower_type ctx in
      ctx |> Ctx.add_item (xs, []) (Ir3.IAbstractDef (loc, d, a, ts, t, efs))
  | IAbstractType (loc, d, a, []) ->
      ctx |> Ctx.add_item (xs, []) (Ir3.IAbstractType (loc, d, a))
  | IType (loc, d, [], t) ->
      let (t, ctx) = ctx |> lower_type t in
...
and lower_expr e ctx =
  match e with
  | EAccessRecord (loc, v1, x) ->
      let e = Ir3.EAccessRecord (loc, v1, x) in
      (e, ctx)
  | ESubset _ -> todo ()
  | EUpdateRecord (loc, v1, x, v2) ->
      let e = Ir3.EUpdateRecord (loc, v1, x, v2) in
      (e, ctx)
  | ELit (loc, l) ->
...
and lower_expr_item xs ts ctx =
  if not (ctx |> Ctx.has_item (xs, ts)) then
    ...
    | IDef (loc, d, gs, ps, t, b) ->
        ...
        ctx
    | IAbstractDef (loc, d, a, gs, ts1, t, efs) ->
        let ctx = ctx |> Ctx.push_iscope gs ts in
        let (ts1, ctx) = ts1 |> mapm lower_type ctx in
        let (t, ctx) = ctx |> lower_type t in
        let ctx = ctx |> Ctx.add_item (xs, ts) (Ir3.IAbstractDef (loc, d, a, ts1, t, efs)) in
        let ctx = ctx |> Ctx.pop_iscope in
        ctx
    | _ ->
        unreachable ()
...
and lower_type_item xs ts (ctx:Ctx.t) =
  if not (ctx |> Ctx.has_item (xs, ts)) then
    match ctx.ir2 |> assoc xs with
    | IAbstractType (loc, d, a, _) ->
        ctx |> Ctx.add_item (xs, ts) (Ir3.IAbstractType (loc, d, a))
    | _ -> unreachable ()
  else
    ctx
