...
module Ctx = struct
  ...
  *)
  ...
  and break_ty info ctx =
    match hd ctx.cstack with
    | CLoop hd -> hd.break_ty
    | _ -> raise (Error.TypingError (info, "Tried to break outside of loop"))
  ...
  and typeof_var info x ctx =
    match ctx |> get_vstack |> find_map (fun s -> s.vsubst |> assoc_opt x) with
    | Some t -> t
    | None -> raise (Error.TypingError (info, "Variable not bound: " ^ x))
...
and instantiate_generic g ctx =
  ...
  if !Args.verbose then begin
    Printf.eprintf "Instantiating generic: ";
...
and debug_substitutions s =
  Printf.eprintf "Substitutions:\n";
  Print.pr_sep "" debug_substitution s Print.Ctx.make;
  Printf.eprintf "\n";

(* Unifies two types *)
and unify info t t1 ctx =
  if !Args.verbose then
    begin
      let ctx = Print.Ctx.make in
      Printf.eprintf "\n";
      Printf.eprintf "Unifying:  (";
  ...
  let (s1, ctx) = ctx |> mgu info (apply s0 t) (apply s0 t1) in
...
and unify_ts info ts0 ts1 ctx =
  if ts0 <> [] && ts1 <> [] then
    zip ts0 ts1 |> foldl (fun ctx (t, t1) -> unify info t t1 ctx) ctx
  else
    ctx
...
and tvs t =
  ...
  if !Args.verbose then begin
    Printf.eprintf "\n";
    Print.Ir1.pr_type t Print.Ctx.make;
    Printf.eprintf " has freevars [%s]" (xs |> String.concat ", ");
    Printf.eprintf "\n";
  end;
  xs

and mgus info ts0 ts1 s ctx =
  if !Args.verbose then begin
    Printf.eprintf "\n[";
...
and mgu info t0 t1 ctx =
  match t0, t1 with
  | TFunc (ts0, t0), TFunc (ts1, t1) ->
      ctx |> mgus info (ts0 @ [t0]) (ts1 @ [t1]) []
  ...
  | TRowExtend ((x0, t0), r0), (TRowExtend _ as r1) ->
      let (t1, r1, s, ctx) = ctx |> rewrite_row info x0 r1 in
      ctx |> mgus info [t0; r0] [t1; r1] s
  | TNominal (xs0, ts0), TNominal (xs1, ts1) when xs0 = xs1 ->
      ctx |> mgus info ts0 ts1 []
  ...
  | _ ->
      ...
      Printf.eprintf "'\n";
      raise (Error.TypingError (info, "Types do not unify\n"))
...
and rewrite_row info x0 r0 ctx =
  let rec rewrite_row r0 ctx =
    ...
    | TVar _ as r1 ->
        ...
        let (s, ctx) = ctx |> mgu info r1 (TRowExtend ((x0, t2), r2)) in
        (t2, r2, s, ctx)
    | _ ->
        raise (Error.TypingError (info, "Cannot unify a type with a row"))
  in
  rewrite_row r0 ctx
...
and infer_item (xs, i) ctx =
  if ctx |> Ctx.has_scheme xs then
    ...
    match i with
    | IDef (info, a, explicit_gs, ps, t, _bs, b) ->
        ...
        let ctx = ctx |> unify info t (typeof_block b) in
        ...
        (* Create the function *)
        let i = IDef (info, a, explicit_gs @ implicit_gs, ps, t, _bs, b) in
        ...
        ctx
    | IAbstractDef (_, _, _, explicit_gs, ts, t, _, _bs) ->
        let t_fun = TFunc (ts, t) in
        let ctx = ctx |> Ctx.add_scheme xs (Ctx.SPoly { t_fun; explicit_gs; implicit_gs = [] }) in
        let ctx = ctx |> Ctx.add_item xs i in
        ctx
    | IClass _ | IVal _ -> todo ()
    | IAbstractType _ | IType _ -> ctx |> Ctx.add_item xs i

and infer_lit info t l ctx =
  match l with
  | LInt (_, _) ->
      ctx |> unify info t (atom "i32")
  | LFloat (_, _) ->
      ctx |> unify info t (atom "f32")
  | LBool _ ->
      ctx |> unify info t (atom "bool")
  | LString _ ->
      ctx |> unify info t (atom "str")
  | LUnit _ ->
      ctx |> unify info t (atom "unit")
  | LChar _ ->
      ctx |> unify info t (atom "char")

and infer_expr e ctx =
  match e with
  | EAccessRecord (info, t, e1, x) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let (r2, ctx) = ctx |> Ctx.fresh_r in
      let ctx = ctx |> unify info (TRecord (TRowExtend ((x, t), r2))) (typeof_expr e1) in
      (EAccessRecord (info, t, e1, x), ctx)
  | EUpdateRecord (info, t, e1, x, e2) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let (e2, ctx) = ctx |> infer_expr e2 in
      let (r2, ctx) = ctx |> Ctx.fresh_r in
      let ctx = ctx |> unify info t (atom "unit") in
      let ctx = ctx |> unify info (TRecord (TRowExtend ((x, (typeof_expr e2)), r2))) (typeof_expr e1) in
      (EUpdateRecord (info, t, e1, x, e2), ctx)
  | ERecord (info, t, (xes, e)) ->
      ...
      let (r, ctx) = match e with
        ...
        | Some e ->
            ...
            let ctx = ctx |> unify info (TRecord r) (typeof_expr e) in
            (r, ctx)
      in
      let ctx = ctx |> unify info t (TRecord (xts |> fields_to_rows r)) in
      (ERecord (info, t, (xes, e)), ctx)
  | EEnwrap (info, t, x, e1) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let ctx = ctx |> unify info t (TEnum ([(x, typeof_expr e1)] |> fields_to_rows TRowEmpty)) in
      (EEnwrap (info, t, x, e1), ctx)
  | EAnnot (info, t, e1) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let ctx = ctx |> unify info t (typeof_expr e1) in
      (EAnnot (info, t, e1), ctx)
  | ELit (info, t, l) ->
      let ctx = ctx |> infer_lit info t l in
      (ELit (info, t, l), ctx)
  | ELoop (info, t, b) ->
      let ctx = ctx |> Ctx.push_subctx (Ctx.CLoop { break_ty=t }) in
      let (b, ctx) = ctx |> infer_block b in
      let ctx = ctx |> unify info (typeof_block b) (atom "unit") in
      let ctx = ctx |> Ctx.pop_subctx in
      (ELoop (info, t, b), ctx)
  | EReturn (info, t, e1) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let ctx = ctx |> unify info (typeof_expr e1) (ctx |> Ctx.return_ty) in
      (EReturn (info, t, e1), ctx)
  | EBreak (info, t, e1) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let ctx = ctx |> unify info (typeof_expr e1) (ctx |> Ctx.break_ty info) in
      (EBreak (info, t, e1), ctx)
  | EContinue (info, t) ->
      (EContinue (info, t), ctx)
  ...
  *)
  | EItem (info, t, xs, ts) ->
      let (ts, ctx) = infer_item_expr info t (xs, ts) ctx in
      (EItem (info, t, xs, ts), ctx)
  | ECallExpr (info, t, e1, es) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let (es, ctx) = es |> mapm infer_expr ctx in
      let ctx = ctx |> unify info (typeof_expr e1) (TFunc (ts_of_es es, t)) in
      (ECallExpr (info, t, e1, es), ctx)
  | ECallItem (info, t, xs, ts, es) ->
      let (es, ctx) = es |> mapm infer_expr ctx in
      let (ts, ctx) = infer_item_expr info (TFunc (ts_of_es es, t)) (xs, ts) ctx in
      (ECallItem (info, t, xs, ts, es), ctx)
  | EVar (info, t, x) ->
      let t1 = (ctx |> Ctx.typeof_var info x) in
      let ctx = ctx |> unify info t t1 in
      (EVar (info, t, x), ctx)
  | EMatch (info, t1, e1, arms) ->
      let (e1, ctx) = ctx |> infer_expr e1 in
      let (arms, ctx) = arms |> mapm (infer_arm (typeof_expr e1) t1 info) ctx in
      (EMatch (info, t1, e1, arms), ctx)
...
and infer_arm t1 t2 info (p, b) ctx =
  let (p, ctx) = ctx |> infer_pat p in
  let (b, ctx) = ctx |> infer_block b in
  let ctx = ctx |> unify info t1 (typeof_pat p) in
  let ctx = ctx |> unify info t2 (typeof_block b) in
  ((p, b), ctx)
...
and infer_pat p ctx =
  ...
  | POr (info, t, p0, p1) ->
      ...
      let ctx = unify info t (typeof_pat p0) ctx in
      let ctx = unify info t (typeof_pat p1) ctx in
      (POr (info, t, p0, p1), ctx)
  | PAnnot (info, t, p0) ->
      let (p0, ctx) = ctx |> infer_pat p0 in
      let ctx = unify info t (typeof_pat p0) ctx in
      (PAnnot (info, t, p0), ctx)
  | PRecord (info, t, (xps, p1)) ->
      ...
      let (r, ctx) = match p1 with
        ...
        | Some p1 ->
            let (r, ctx) = ctx |> Ctx.fresh_r in
            let ctx = ctx |> unify info (TRecord r) (typeof_pat p1) in
            (r, ctx)
      in
      let ctx = ctx |> unify info t (TRecord (xts |> fields_to_rows r)) in
      (PRecord (info, t, (xps, p1)), ctx)
  | PConst (info, t, c) ->
      let ctx = ctx |> infer_lit info t c in
      (PConst (info, t, c), ctx)
  | PUnwrap (info, t, x, p1) ->
      let (p1, ctx) = ctx |> infer_pat p1 in
      let (r, ctx) = ctx |> Ctx.fresh_r in
      let ctx = ctx |> unify info t (TEnum r) in
      let ctx = ctx |> unify info t (TEnum ([(x, typeof_pat p1)] |> fields_to_rows r)) in
      (PUnwrap (info, t, x, p1), ctx)

and infer_item_expr info t (xs, ts0) ctx =
  let i = ctx.ir1 |> get_item info xs in
  ...
  ** its type scheme *)
  | Ctx.SPoly sc ->
      ...
      let ctx = ctx |> unify_ts info ts0 ts1 in
